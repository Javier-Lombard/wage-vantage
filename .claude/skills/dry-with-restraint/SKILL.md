...
description: Rule-of-three duplication discipline for React 19 + TypeScript + RTK/RTK Query in this codebase's feature-based architecture. Use before writing or substantially modifying any non-trivial component, hook, slice, or endpoint — any new file inside `src/features/<name>/` (components, hooks, api, utils), any new custom hook, any new RTK slice or RTK Query endpoint, any component that fetches data or owns form state, any component with more than ~3 props or visible conditional rendering, and any decision about extracting a hook or shared component out of duplicated logic. Skip for purely additive changes to atoms in `src/shared/components/ui/` (new variant, new color, new icon). Distinction from the other two design skills: `solid-principles` asks whether a unit's internal responsibilities are well split; `separation-of-concerns` asks which layer/location a piece of code belongs to; this skill asks whether near-identical logic already exists elsewhere and, if so, whether *now* (third occurrence) is the right moment to collapse it — and warns against collapsing on the first or second. Defers to docs/architecture.md and docs/conventions.md as authoritative — this skill operationalizes those documents, it never overrides them.
---
