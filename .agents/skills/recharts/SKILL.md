---
name: recharts
description: Build composable, responsive React charts with Recharts library. Use when creating data visualizations including line charts, area charts, bar charts, pie charts, scatter plots, and composed charts. Handles chart customization, responsive sizing, tooltips, legends, axes configuration, performance optimization, and accessibility. For box plots specifically — Recharts has no native BoxPlot component — see references/boxplot-pattern.md in addition to references/examples.md.
---
...
> The examples throughout this `SKILL.md` and its `references/` files are JSX, as downloaded from the external skill source. This section is the only part of this file written for this project — it does not modify the JSX examples above or below, it adds typing guidance on top of them. `references/boxplot-pattern.md` follows the same addendum convention with TypeScript throughout, for the same reason.

Recharts ships its own type definitions (no `@types/recharts` needed) — `interface`s for chart props, `event` handler payloads, and `dataKey`/`shape` callback args are already typed by the library. A few patterns recur when adapting these JSX examples to this project's `verbatimModuleSyntax`-strict TypeScript:

- **Type the data array explicitly**, don't let it infer from an inline literal — `const data: WageRecord[] = [...]` instead of `const data = [...]`, so a typo in a `dataKey` string is caught at the call site instead of silently rendering nothing.
- **Custom `shape`/`content`/`label` render-prop functions** (e.g. a custom `Tooltip` content or `Bar` shape) receive a Recharts-defined props object — type the function's parameter against that shape rather than `any`, per this project's no-`any` convention. See `references/boxplot-pattern.md`'s `BarShapeProps` for a worked example of this for `Bar`'s `shape` prop specifically.
- **Event handlers** (`onClick`, `onMouseEnter` on a `Bar`/`Line`/`Pie`) receive `(data, index, event)` — destructure and type only the fields actually used; do not retype Recharts' own payload shape from scratch.
- **Generic data over `any[]`**: if a chart-wrapping component in this codebase is meant to be reused across more than one data shape (rare — most charts here are feature-specific per `docs/architecture.md` §2), type it with a generic (`function StatChart<T extends { label: string }>(...)`) rather than loosening to `unknown[]`/`any[]`.
